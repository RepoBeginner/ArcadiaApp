    ...
    var body: some View {
        List {
            ...
            Section(header: Text("Introduction")) {
                Text("""
                Altea is a frontend for multiple emulators (also called cores), each corresponding to a different game system. Specifically, Altea is a frontend for the Libretro API, using the libretro versions of the cores. It aims to provide a simple and accessible way to enjoy retro game systems, not to replace RetroArch. If you are tech-savvy and want more control, consider using the RetroArch app.
                
                In Altea, you'll find only one core per game system, selected by me based on various criteria. Customization of core options is not available (though this may change in future versions).
            ...
            Section(header: Text("Games")) {
                Section(header: Text("Organization")) {
                    Text("""
                    The first time Altea starts, it will create a folder to store all documents. On a Mac, you can find this inside the Documents folder, and on an iPhone, in the Files app.
                ...
                Section(header: Text("Synchronization")) {
                    ...
                    When iCloud sync is enabled, the iCloud Drive Altea folder is updated through two processes: periodic syncs and specific actions.
                    During periodic syncs, the iCloud Drive Altea folder acts as the master. If the iCloud folder contains a file that the local folder does not, it will be downloaded. Conversely, if the local folder contains a file not present in the iCloud folder, it will be deleted. All files that exist in both locations will be kept in sync, with the most recent versions replacing older ones.
                ...
                Section(header: Text("Where to Find Games?")) {
                    Text("""
                    In the featured games section you'll find some game provided by some developers who decided to partner with Altea to distribute their games through the app.
            ...
            Section(header: Text("Controls")) {
                ...
                Text("""
                     ...
                     Altea supports local multiplayer for games designed for more than two players. In the settings or from the in-game menu (accessible by clicking the Altea button), you can see the player mapped for touch input and each game controller, if connected. By default, touch input controls player one, the first controller controls player one, and each additional controller controls the next available player.
                     
                     """)
            }
            Section(header: Text("Saves and States")) {
                Text("""
                Altea and its emulators allow you to save your progress in two ways:
